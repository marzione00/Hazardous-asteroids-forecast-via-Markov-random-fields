...
We started the analysis with the graphical models by inspecting the relations between continuous variables in the dataset, thus in this first step the binary variable \textit{Hazardous} was excluded. For this purpose, among the different methods available, we considered the \textit{Graphical least absolute shrinkage and selection operator} GLASSO as implemented in the \textit{glasso} R package \cite{friedman2008sparse,glasso}. After different tests reported in Fig. \ref{GLASSO_convergence}, we considered as final result the graph obtained with the value of $\rho$ (the one that penalize further connections) equal to $0.2$. In this plot we see that the conditional dependences/independences state by the Celestial Mechanics are reproduced: for instance we see that the mean motion is conditionally independent with respect to the the estimated diameters, but the relative velocity not. Furthermore the eccentricity is conditionally depended, as one can expect to quantity that describe the elliptical orbits: the perihelion distance, the semi-major axis and the Jupiter Tisserand's parameter. Finally we would compare the results of GLASSO with the graph intersection obtained from the stepwise model (forward direction, BIC) (as implemented in the grim package \cite{hojsgaard2012graphical}), SIN (simultaneous p-values method, as implemented in the SIN package \cite{SIN_package}) and thresholding method (as implemented in the grim package \cite{hojsgaard2012graphical}) shown in Fig. \ref{ST+TH+SIN_PLOT}: in this case the volume/mass of the asteroid is not connect with the Relative velocity.
...
\begin{figure}
\begin{center}
\includegraphics[\linewidth]{Figures/ST+TH+SIN_PLOT.pdf}
\caption{Intersection of the graphs obtained from the stepwise method (forward, BIC),from the  threshold method (as implemented in the grim package \cite{hojsgaard2012graphical}) and the SIN one (as implemented in the SIN package \cite{SIN_package}). The plots were obtained with the igraph package for R \cite{igraph} }
\label{ST+TH+SIN_PLOT}
\end{center}
\end{figure}
